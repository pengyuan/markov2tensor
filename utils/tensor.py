...
def hosvd(T, threshold=1, saveSpace=False):
    ...
    for n in range(T.ndim):
        ...
        print "S.shape: ", S.shape, len(S)
        sum = 0
        for value in S:
            sum += value
        thres_sum = sum * threshold

        print sum, threshold, thres_sum
        ...
        for value in S:
            value_sum += value
            count += 1
            if value_sum >= thres_sum:
                break

        print "count: ", count
        print U.shape
        Un.append(U)
        new_Un.append(U[:, :count])
        if not saveSpace:
            #Vn.append(V)
            #Sn.append(S)
            new_Sn.append(S[:count])

    core = T.copy()
    for n in range(len(new_Un)):
        core = nmodmult(core, new_Un[n].T, n+1)
    for n in range(len(new_Un)):
        new_T = nmodmult(core, new_Un[n], n+1)

    core2 = T.copy()
    for n in range(len(Un)):
        core2 = nmodmult(core2, Un[n].T, n+1)
    for n in range(len(Un)):
        T = nmodmult(core2, Un[n], n+1)

    if not saveSpace:
        return [new_T, T, core, new_Un, new_Sn, Vn]
    return [new_T, core, new_Un]
